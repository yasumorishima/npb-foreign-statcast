...
- 2026-09-25: rebuilt all pillar-1 inputs in cloud (new `src/fetch_bdata.py`), ana1.py reproduced RESULTS_pillar1_run1.txt exactly; ran S1v2 once: bat AUCw wOBA 0.333 / xwOBA 0.609 (p_BA .022, B vs 0.5 p .14), pit K−BB% 0.610 / CSW% 0.585 (p .63) → registered rule not met. Next: 2b, or task 1 once the season ends.
...
2. **S1 replacement: done** (2026-09-25, `results/RESULTS_S1v2_run1.txt`, null by the registered rule for both roles). Do not rerun.
   Cloud rebuild recipe (worked 2026-09-25, ~2.5 h mostly fetching; `pip install pyarrow "scikit-learn<1.10"`): Chadwick `people-{0..f}.csv` from raw.githubusercontent.com into `~/claude-scratch/npbid/`; in a scratch dir run `fetch_bdata.py`, `fetch_npb_pages.py` (~1 h), `link_foreign.py` (now 313 rows / 312 players vs original 312/311, Chadwick grew; no effect on entrants), `mkdir mlb; fetch_mlb.py` (653 files, ~1 h), then `ana1.py` with `PREREG_pillar1.md` copied in — its output must equal `results/RESULTS_pillar1_run1.txt` (it did). Network needs `huggingface.co` + `us.aws.cdn.hf.co`.
